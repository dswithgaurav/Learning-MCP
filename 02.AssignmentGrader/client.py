...
st.session_state["openai_api_key"] = "1"
st.session_state["google_api_key"] = "2"
st.session_state["google_cx"] = "3"
...
def call_api_tool(tool_name, data):
    ...
    request_data["google_api_key"] = ""
    request_data["search_engine_id"] = ""
...
st.info(
    "This version has hardcoded API keys for debugging purposes. API keys are automatically included in all requests."
)
...
with st.sidebar.expander("Server Settings", expanded=True):
    ...
    server_url = st.text_input(
        "API Server URL", value=st.session_state["api_server_url"]
    )
            ...
            with st.spinner("Processing document..."):
                ...
                if result is None:
                    ...
                    with st.expander("Document Preview"):
                        ...
                        st.text_area(
                            "Preview", value=preview, height=300, disabled=True
                        )

                    # If document is very long, show a warning
                    if word_count > 5000:
                        st.warning(
                            f"Long document detected ({word_count} words). Processing might take longer."
                        )
                else:
...
Language & Style (20%): The language should be engaging, varied, and appropriate.""",
    ...
    default_value = (
        rubric_templates.get(template_choice, "") if template_choice != "Custom" else ""
    )
        ...
        if st.button("Grade Assignment", type="primary"):
            ...
            with st.spinner("Grading in progress..."):
                ...
                grade_data = {
                    ...
                    "rubric": rubric,
                    "model": (
                        grade_model if "grade_model" in locals() else "gpt-3.5-turbo"
                    ),
                }
...
with tab3:
    ...
    if all(k in st.session_state for k in ["file_name"]):
        ...
        with col1:
            if (
                "grade_results" in st.session_state
                and st.session_state["grade_results"] is not None
            ):
                if isinstance(st.session_state["grade_results"], dict):
                    grade = st.session_state["grade_results"].get(
                        "grade", "Not available"
                    )
                else:
        ...
        with col2:
            if (
                "feedback" in st.session_state
                and st.session_state["feedback"] is not None
            ):
        ...
        if (
            "plagiarism_results" in st.session_state
            and st.session_state["plagiarism_results"]
        ):
            ...
            if results is None:
                ...
                for item in results["results"]:
                    ...
                    if similarity > 70:
                        ...
                        st.info(
                            f"ℹ️ Moderate similarity ({similarity}%): [{url}]({url})"
                        )
                    else:
                ...
                if isinstance(results, dict):
                    for url, similarity in results.items():
                        if similarity > 70:
                            st.warning(
                                f"⚠️ High similarity ({similarity}%): [{url}]({url})"
                            )
                        elif similarity > 40:
                            st.info(
                                f"ℹ️ Moderate similarity ({similarity}%): [{url}]({url})"
                            ...
                            st.success(
                                f"✅ Low similarity ({similarity}%): [{url}]({url})"
                            )
                else:
                    # Display raw results if format is unknown
                    st.json(results)
        ...
        st.info(
            "No grading results available. Please upload and grade an assignment first."
        )
